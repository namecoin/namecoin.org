...
* You can keep your `tls` field in your `d/` name (alongside an `import` field that points at a `dd/` name) so that, if the `dd/` name is later stolen, the thief cannot replace the on-chain CA pin and therefore cannot mount a MITM attack -- they can only DoS your site by pointing it at a server they control.  See [Delegated Alteration]({{ "/docs/name-owners/delegated-alteration" | relative_url }}) for the full pattern, including *delegated partial alteration*, which is specifically designed to keep the `tls` field in the `d/` name while letting a hotter `dd/` name update other DNS-style records.
